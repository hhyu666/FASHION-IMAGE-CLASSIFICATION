...
def predict(IMAGE_ROOT):
    ...
    result =''
    for a, b, c in os.walk(IMAGE_ROOT):
        for filei in c:
            full_path = os.path.join(a, filei)
            if full_path.endswith('.png') or full_path.endswith('.PNG'):
                PNG_JPG(full_path.replace("\\", "/"))
                full_path = (full_path[:-3] + "jpg")
                print(full_path)

            if full_path.endswith('.jpg') or full_path.endswith('.JPG'):
            ...
            # resu = "\t\t" + "({})".format((output_label(eval_num_label))) + "\t  With conficence: {0:.4f}%".format(
            #             #     eval_confidence)
            #             # result = "({})".format((output_label(eval_num_label))) + "\nWith conficence: {0:.4f}%\n".format(
            #             #     eval_confidence)lt
                clothes = "({})".format((output_label(eval_num_label)))
                if clothes == '(Bag)':
                    out = filei + ' WrongImageEntered!'
                else:
                    out = filei + " 预测类别为： " + clothes
                result += out + '\n'
